 ...
 * every 5s and auto-allocating toward minCartelas(stake) once at least one
 * real cartela has been sold. Restarts the window (same game document) if
 * the window closes with 0 real cartelas sold.
 */
async function runWaitingPhase(game) {
  for (;;) {
    ...
    for (let tick = 0; tick < totalTicks; tick++) {
      ...
      const { real, total } = await cartelaService.countSold(game.gameId);
      if (real >= 1) realSoldEverThisWindow = true;

      if (realSoldEverThisWindow) {
        const minCartelas = stateMachine.minCartelasForCurrentStake();
        if (total < minCartelas) {
          const ticksLeft = totalTicks - tick - 1;
          const batchSize = scheduler.batchSizeForTick(game.stake, total, Math.max(1, ticksLeft));
          if (batchSize > 0) {
            await withLock(`auto-allocate:${game.gameId}`, 8000, async () => {
              const { allocated } = await cartelaService.allocateToSystemAdmin(game.gameId, batchSize);
              if (allocated > 0) {
                logger.info('Auto-allocated cartelas', { gameId: game.gameId, allocated });
...
async function start() {
  ...
  while (!stopRequested) {
    try {
      ...
      const missingTemplates = /No cartela templates found/.test(err.message);
      if (missingTemplates) {
        logger.error(
          'Cannot start a round: no cartela templates in the database yet. ' +
          'Run `npm run import:cartelas` — the engine will pick it up automatically on its next retry (30s).'
        );
        await sleep(30000);
      } else {
